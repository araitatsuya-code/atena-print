...
## Phase 7: 印刷品質改善 (次フェーズ優先)

### 7.1 ラベル横書き対応 (Issue #30)
- [ ] テンプレート/設定UIに「縦書き・横書き」切替を追加
- [ ] プレビュー（Canvas）で `orientation=horizontal` の描画ロジックを実装
- [ ] PDF生成（gofpdf）で `orientation=horizontal` の描画を実装
- [ ] 印刷ジョブ生成で `orientation` 固定値 (`vertical`) を廃止
- [ ] 縦書き/横書きのプレビュー・PDF一致確認と回帰テスト

### 7.2 ラベル印刷位置のドラッグ微調整 (Issue #31)
- [ ] プレビュー上でドラッグによる位置調整UIを追加
- [ ] X/Y オフセット（mm）表示と数値入力を追加
- [ ] オフセット保存（再起動後の再利用）を実装
- [ ] PDF生成側にも同一オフセットを適用
- [ ] 「初期値に戻す」導線を追加
